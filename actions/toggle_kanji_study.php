...
$entry = $stmt->fetch();
if (!$entry) {
    exit("No such kanji found.");
}

// if its already added remove and reset score
if ($entry["added"] == 1) {
    $sql =
        "UPDATE kanjis SET added = 0, score = 0, added_at = null WHERE literal = ?";
} else {
    $sql =
        "UPDATE kanjis SET added = 1, score = 0, unfinished = 1, added_at = CURRENT_TIMESTAMP WHERE literal = ?";
}
$stmt = $myPDO->prepare($sql);
$results = $stmt->execute([$_POST["literal"]]);
if (!$results) {
    exit("Error toggling kanji " . $_POST["literal"]);
}
